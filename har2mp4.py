...
def parseHAR(f):
    ...
    for e in data["log"]["entries"]:
        ...
        m3uList.append([
            ...
            base64.b64decode(e["response"]["content"]["text"]).decode("utf-8")
        ])
...
def splitTsInfo(s):
    ...
    for l in s.splitlines():
        # 跳过文件头，至到出现 "#EXTINF"
        if isHead and l.startswith("#EXTINF:"):
            isHead = False
        if isHead: continue
...
# 返回 cur 中，不与 prev 重得的项（复制 cur 列表，不改变原始 cur）
...
def outputMulti(data):
    ...
    for k, v in data["ts"].items():
        with open(k, "wb") as tsf: tsf.write(v)
...
def outputMp4(data, fileName):
    ...
    # 转 o.ts -> o.mp4
    cmd = f"ffmpeg -i o.ts -c copy {fileName}"
    os.system(cmd)
